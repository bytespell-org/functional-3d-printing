...
1. **Capture truth.** Put supplied dimensions in one `known_dimensions_mm` map. Model every fit-controlling item as a non-printable `ReferenceComponent`: prefer manufacturer CAD, otherwise use a conservative measured or nominal envelope. Include plugs, wire exits, bend space, tools, fastener heads, and moving hardware—not only the installed component.
...
10. **Share observably.** Start `scripts/serve_preview.py <output>/preview --daemon`, verify the viewer, model URLs, progress, and comment create/delete flow, then give the LAN URL. After physical feedback, record one observation and the smallest responsible next change in the iteration JSONL.
